...
async function rejectProposal(rejectProposal) {
    ...
    const proposalRegistry = await getAssetRegistry('org.hell.devil.Proposal');
...
async function takeSoul(takeSoul) {
    ...
    souls.push(soul);
    devil.souls = souls;
    let devilRegistry = await getAssetRegistry('org.hell.devil.Devil');
    await devilRegistry.update(deal);
}
